...
class Discount extends \Magento\Quote\Model\Quote\Address\Total\AbstractTotal
{
    ...
    public function collect(
        ...
        $address->setDiscountAmount($address->getDiscountAmount() - $purchase->getSpendAmount());
    ...
    private function spendPoints($total, $shippingAssignment)
    {
        ...
        $baseRoundedItemDiscount = 0;
        $roundedItemDiscount = 0;
        ...
        foreach ($items as $item) {
            ...
            $this->process();
            $baseRoundedItemDiscount += round($this->item->getBaseRewardsDiscountAmount(), 2);
            $roundedItemDiscount += round($this->item->getRewardsDiscountAmount(), 2);
        }
        ...
        $baseDiff = $baseRoundedItemDiscount - $this->baseUsed + $total->getRewardsBaseShippingDiscount();
        $diff = $roundedItemDiscount - $this->used + $total->getRewardsShippingDiscount();
        if (abs($baseDiff) >= 0.0099 || abs($diff) >= 0.0099) {
            $this->fixItemDiscount($baseDiff, $diff, $items);
        }
    ...
    private function fixItemDiscount($baseDiff, $diff, $items): void
    {
        $baseDiff              = round($baseDiff, 2);
        $diff                  = round($diff, 2);
        $fixBaseDiscountAmount = $baseDiff;
        $fixDiscountAmount     = $diff;
        foreach ($items as $item) {
            if ($item->getBaseRewardsDiscountAmount() && $baseDiff < 0 && $fixBaseDiscountAmount != 0) {
                $item->setBaseDiscountAmount(round($item->getBaseDiscountAmount() - $baseDiff, 2));
                $fixBaseDiscountAmount = 0;
            }

            if ($item->getBaseRewardsDiscountAmount() && $baseDiff > 0
                && $item->getRewardsBaseTotalPrice() - $item->getBaseDiscountAmount() + $baseDiff > 0.0099) {
                if ($item->getBaseDiscountAmount() + $baseDiff - $item->getRewardsBaseTotalPrice() <= $fixBaseDiscountAmount
                    && $fixBaseDiscountAmount != 0) {
                    $item->setBaseDiscountAmount(round($item->getBaseDiscountAmount() - $baseDiff, 2));
                    $fixBaseDiscountAmount = 0;
                }
            ...
            if ($item->getRewardsDiscountAmount() && $diff < 0 && $fixDiscountAmount != 0) {
                $item->setDiscountAmount(round($item->getDiscountAmount() - $diff, 2));
                $fixDiscountAmount = 0;

            }

            if ($item->getRewardsDiscountAmount() && $diff > 0
                && $item->getRewardsTotalPrice() - $item->getDiscountAmount() + $diff > 0.0099) {
                if ($item->getDiscountAmount() + $diff - $item->getRewardsTotalPrice() <= $fixDiscountAmount
                    && $fixDiscountAmount != 0) {
                    $item->setDiscountAmount(round($item->getDiscountAmount() - $diff, 2));
                    $fixDiscountAmount = 0;
                }
            ...
            $item->setBaseDiscountAmount(round($item->getBaseDiscountAmount(), 2));
            $item->setDiscountAmount(round($item->getDiscountAmount(), 2));
        }
    }
    ...
    private function processShipping($total)
    {
        ...
        //We shouldn't add shipping Tax if it is already in the shipping amount
        if (!$this->taxConfig->getShippingTaxClass()) {
        ...
        $total->setRewardsShippingDiscount(round($shippingDiscount, 2));
        $total->setRewardsBaseShippingDiscount(round($baseShippingDiscount, 2));
    ...
    private function canProcess()
    {
        if ($this->item->getParentItem()
            && $this->item->getParentItem()->getProduct()->getTypeId() != 'bundle'
        ) {
            return false;
        }
    ...
    private function roundPriceWithFaonniPrice($price)
    {
        ...
        $math          = $objectManager->create('Faonni\Price\Model\Calculator\RoundProcessor\Excel\RoundProcessor');
        if (!$helper->isEnabled() ||
