...
class Rule extends \Magento\SalesRule\Model\Rule
{
     ...
     * @return Tier
     */
    public function getTier($customer): Tier
    {
        ...
        if ($currentTierId) {
            ...
            $tierCollection = $this->tierRepository->getCollection();
            foreach ($tiers as $key => $value) {
                $tierData = [];
                foreach ($tierCollection as $tier) {
                    if ($tier->getId() == $key
                        && in_array($this->_storeManager->getWebsite()->getId(), explode(',', $tier->getWebsiteIds()))
                        && $tier->getMinEarnPoints() == 0) {
                        $tierData = $value;
                        break;
                    }
                }
