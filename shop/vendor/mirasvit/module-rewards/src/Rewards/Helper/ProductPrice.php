...
class ProductPrice
{
     ...
     * @return float
     */
    public function getProductPrice(Product $product, $priceType = 'final_price'): float
    {
        ...
        if ($bundleProduct) {
            foreach ($bundleProduct->getRelatedProducts() as $relatedProduct) {
                $relatedProdIds[] = $relatedProduct->getId();
            }
            foreach ($bundleProduct->getUpSellProducts() as $upselProduct) {
                $upselProductIds[] = $upselProduct->getId();
            }
        }
        ...
        if ($this->isBundle($product)) {
            ...
            && !in_array($product->getId(), array_merge($upselProductIds, $relatedProdIds))
        ) {
            // bundle options
    ...
    private function getBundlePrice(Product $product, $priceType)
    {
        ...
        if ($product->getFinalPrice()) {
            $bundlePrice += $product->getFinalPrice();
        }
