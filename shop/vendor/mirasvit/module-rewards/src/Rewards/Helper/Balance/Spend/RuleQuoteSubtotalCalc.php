...
class RuleQuoteSubtotalCalc
{
    ...
    public function getLimitedSubtotal($quote, $rule)
    {
        ...
        foreach ($quote->getItemsCollection() as $item) {
            if ($this->shouldNotCalculate($item)) {
                continue;
            }
            /** @var \Magento\SalesRule\Model\Rule\Condition\Product\Combine $actions */
            $actions = $rule->getActions();
            if ($actions->validate($this->getProductByItem($item))) {
    ...
    private function shouldNotCalculate($item): bool
    {
        return ($item->getParentItemId() && $item->getParentItem()->getProduct()->getTypeId() != 'bundle')
               || ($item->getProduct()->getTypeId() == 'bundle' && !$item->getParentItemId()
