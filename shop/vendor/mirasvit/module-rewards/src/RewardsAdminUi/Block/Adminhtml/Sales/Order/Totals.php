...
class Totals extends \Magento\Sales\Block\Adminhtml\Order\Totals
{
    ...
    protected function initTotals()
    {
        ...
        if ($purchase->getSpendAmount() > 0) {
            ...
            if ($order->getDiscountInvoiced() && $block->getTotal('discount')
                ...
                if (count($order->getAllItems()) > count($this->getSource()->getAllItems())) {
                    $block->getTotal('discount')
                        ->setLabel(
                            str_replace(')', ', ' . $this->rewardsData->getPointsName().')',
                            (string)$block->getTotal('discount')->getLabel())
                        );
                    $block->removeTotal('spent');
                } else {
                    $discountTotal = $block->getTotal('discount');
                    $block->removeTotal('discount');
                    $discountTotal->setValue($order->getDiscountInvoiced() + $purchase->getSpendAmount());
                    $block->addTotalBefore($discountTotal, 'tax');
                }
            }
        }
